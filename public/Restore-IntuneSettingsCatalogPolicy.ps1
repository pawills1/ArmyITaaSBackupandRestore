function Restore-IntuneSettingsCatalogPolicy {
   <#
.Synopsis
   Restores Intune Settings Catalog policies.
...
.DESCRIPTION
   Restores Intune Settings Catalog policies from JSON backup files.
...
.EXAMPLE
   Restore-IntuneSettingsCatalogPolicy -Path C:\Backup\SettingsCatalog

.EXAMPLE
   # Apply the T2COM scope tag to every restored policy
   Restore-IntuneSettingsCatalogPolicy -Path C:\Backup\SettingsCatalog -ScopeTagName T2COM

.EXAMPLE
   # Apply the GCC scope tag to every restored policy
   Restore-IntuneSettingsCatalogPolicy -Path C:\Backup\SettingsCatalog -ScopeTagName GCC

.EXAMPLE
   Restore-IntuneSettingsCatalogPolicy -Path C:\Backup\SettingsCatalog -ScopeTagName AVD
...
#>
   ...
   if ([string]::IsNullOrWhiteSpace($ScopeTagName)) {
      ...
      Write-Host '    Restore-IntuneSettingsCatalogPolicy -Path <path> -ScopeTagName T2COM' -ForegroundColor Gray
      Write-Host '    Restore-IntuneSettingsCatalogPolicy -Path <path> -ScopeTagName GCC'   -ForegroundColor Gray
      Write-Host '    Restore-IntuneSettingsCatalogPolicy -Path <path> -ScopeTagName PAW'   -ForegroundColor Gray
      Write-Host '    Restore-IntuneSettingsCatalogPolicy -Path <path> -ScopeTagName AVD'   -ForegroundColor Gray
      Write-Host '========================================================' -ForegroundColor Yellow
      Write-Host ''
   ...
   $transcriptPath = Join-Path -Path $Path -ChildPath "Restore-IntuneSettingsCatalogPolicy-$date.log"
   ...
   foreach ($config in $jsonFiles) {
      ...
      if ($match) {
         ...
         $configDisplayName = $configHash['Name']
         ...
         # Sending empty roleScopeTagIds assigns the Default scope, which a PAG/scoped admin
         # cannot see — causing HTTP 403. Include the resolved tag ID here to avoid that.
         ...
         try {
            ...
            Write-Host "Successfully restored settings catalog policy: '$configDisplayName'." -ForegroundColor Green
         }
         catch {
            Write-Verbose "$configDisplayName - Failed to restore Settings Catalog policy." -Verbose
            Write-Error $_ -ErrorAction Continue
         }
      }
      else {
         Write-Warning "Skipping '$($config.Name)' — it does not appear to be a settings catalog policy. Use the appropriate restore function for this configuration type."
      }
   }
