...
.DESCRIPTION
   This function restores Device Configurations
...
#>
   ...
   $transcript = "Restore-IntuneDeviceConfiguration-$date.log"
   $transcriptpath = "$path\$transcript"
   start-transcript -Path $transcriptpath -Append  -Force

   #get all device deviceConfigurations
   #might use later
   #$configurations = (Get-MgBetaDeviceManagementDeviceConfiguration -All).DisplayName
   $deviceConfiguration = Get-ChildItem $path -Include "*.json" -Recurse
   ...
   foreach ($config in $deviceConfiguration) {
      ...
      If ($match) {
         ...
         Write-Warning "The following objects are not device configurations and will not be restored please use the correct restore function for the configuration type $config"
      }
   }
