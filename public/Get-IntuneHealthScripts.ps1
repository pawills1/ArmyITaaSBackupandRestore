function Get-IntuneHealthScripts
{
   <#
.Synopsis
   Export Health Scripts from Intune
...
.DESCRIPTION
   This function exports Health Scripts
...
.EXAMPLE
   Get-IntuneHealthScripts -ScriptNames ENT -outputpath C:\repo\test

.EXAMPLE
   Get-IntuneHealthScripts -ScriptNames PAW-ENT-WIN-Remove M365 Apps -outputpath C:\repo\test
...
$transcript = "Get-IntuneHealthScripts-$date.log"
$transcriptpath = "$outputpath\$transcript"
start-transcript -Path $transcriptpath -Append  -Force

#Get all Health scripts
$healthScripts = Get-MgBetaDeviceManagementDeviceHealthScript -All
$Names = $healthScripts |Where-Object { $_.DisplayName -match $ScriptNames }

if($Names.count -eq 0) {
   #if no scripts are found
   Write-Warning 'You need to enter a valid Script name'  -WarningAction Continue
   Write-host "Setting not found enter a valid name of a configuraton. Example: Get-IntuneHealthScripts -ScriptNames ENT -outputpath C:\repo\test" -ForegroundColor Cyan
   Write-host "Setting not found enter a valid name of a configuraton. Example: Get-IntuneHealthScripts -ScriptNames  PAW-ENT-WIN-Remove M365 Apps  -outputpath C:\repo\test" -ForegroundColor Cyan
   }
      ...
      foreach ($name in $names){
            ...
            $id = $name.id
            $uri = "https://dod-graph.microsoft.us/beta/deviceManagement/deviceHealthScripts/$Id"
