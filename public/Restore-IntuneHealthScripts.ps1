function Restore-IntuneHealthScripts {
   <#
.Synopsis
   Restores Intune Device Health Scripts (Endpoint Analytics remediation scripts).
...
.DESCRIPTION
   Restores Intune Device Health Scripts from JSON backup files.
...
.PARAMETER ScopeTagName
   Optional. The display name of the Role Scope Tag to apply to each restored health script.
   Accepted values: T2COM, GCC, PAW, AVD.
   #If omitted you are prompted interactively; press Enter to skip and use the default scope.

.EXAMPLE
   Restore-IntuneHealthScripts -Path C:\Backup\HealthScripts

.EXAMPLE
   # Apply the T2COM scope tag to every restored health script
   Restore-IntuneHealthScripts -Path C:\Backup\HealthScripts -ScopeTagName T2COM

.EXAMPLE
   # Apply the GCC scope tag to every restored health script
   Restore-IntuneHealthScripts -Path C:\Backup\HealthScripts -ScopeTagName GCC

.EXAMPLE
   Restore-IntuneHealthScripts -Path C:\Backup\HealthScripts -ScopeTagName PAW
...
#>
   ...
   Param (
      ...
      # Optional Role Scope Tag name to apply to each restored health script.
   ...
   if ([string]::IsNullOrWhiteSpace($ScopeTagName)) {
      ...
      Write-Host '    Restore-IntuneHealthScripts -Path <path> -ScopeTagName T2COM' -ForegroundColor Gray
      Write-Host '    Restore-IntuneHealthScripts -Path <path> -ScopeTagName GCC'   -ForegroundColor Gray
      Write-Host '    Restore-IntuneHealthScripts -Path <path> -ScopeTagName PAW'   -ForegroundColor Gray
      Write-Host '    Restore-IntuneHealthScripts -Path <path> -ScopeTagName AVD'   -ForegroundColor Gray
      Write-Host '========================================================' -ForegroundColor Yellow
      Write-Host ''
   ...
   $transcriptPath = Join-Path -Path $Path -ChildPath "Restore-IntuneHealthScripts-$date.log"
   ...
   foreach ($script in $jsonFiles) {
      ...
      if ($match) {
         ...
         $body = @{
            ...
            detectionScriptContent   = $scriptHash['detectionScriptContent']
            remediationScriptContent = $scriptHash['remediationScriptContent']
            runAsAccount             = 'system'
         }
         ...
         try {
            ...
            Write-Host "Successfully restored health script: '$scriptDisplayName'." -ForegroundColor Green
         }
         catch {
            Write-Verbose "$scriptDisplayName - Failed to restore Health Script." -Verbose
            Write-Error $_ -ErrorAction Continue
         }
      }
      else {
         Write-Warning "Skipping '$($script.Name)' — it does not appear to be a health script. Use the appropriate restore function for this configuration type."
      }
   }
